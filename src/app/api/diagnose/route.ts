...
- 「減点します」「不足しています」といった否定的な言葉は避け、「〜を設定することでさらにAIに推薦されやすくなります」「〜がないのは非常にもったいない機会損失です」といった、プロらしいポジティブで寄り添った表現を使用してください。
...
`;
    ...
    if (resend) {
      ...
      const detailsHtml = report.details.map((d: any) => `
        <tr>
          <td style="padding: 14px 10px; border-bottom: 1px solid #333; color: #fff; font-weight: bold; width: 70%;">${d.category}</td>
          <td style="padding: 14px 10px; border-bottom: 1px solid #333; color: #aaa; text-align: right; width: 30%;"><span style="color: #fff; font-size: 18px;">${d.score}</span> / 20</td>
        </tr>
        <tr>
          <td colspan="2" style="padding: 8px 10px 20px; border-bottom: 1px solid #444; color: #999; font-size: 14px; line-height: 1.6;">${d.reason}</td>
        </tr>
      `).join('');

      const actionsHtml = report.actions.map((a: string, i: number) => `
        <div style="background-color: #222; margin-bottom: 10px; padding: 15px; border-radius: 8px; color: #ddd; line-height: 1.5; border-left: 4px solid #fff;">
          <strong style="color: #fff; margin-right: 5px;">Action 0${i+1}.</strong> ${a}
        </div>
      `).join('');

      const htmlTemplate = `
      <div style="font-family: 'Helvetica Neue', Arial, sans-serif; background-color: #000; color: #f5f5f5; padding: 40px 10px; text-align: center;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #111; border-radius: 16px; padding: 40px 30px; text-align: left; border: 1px solid #222;">
          <h2 style="color: #fff; margin-bottom: 4px; font-weight: 400; letter-spacing: 2px;">kuuchuu8sk</h2>
          <h3 style="color: #666; margin-top: 0; font-size: 13px; font-weight: 300; letter-spacing: 1px; margin-bottom: 40px;">AI POWERED ANALYSIS RESULT</h3>
          
          <div style="text-align: center; margin-bottom: 40px; padding: 30px 20px; background: linear-gradient(145deg, #1a1a1a 0%, #0d0d0d 100%); border-radius: 12px; border: 1px solid #222;">
            <p style="color: #888; margin-bottom: 10px; font-size: 12px; letter-spacing: 2px; text-transform: uppercase;">Diagnostic Score for ${placeData.name}</p>
            <div style="font-size: 72px; color: #fff; font-weight: 200; line-height: 1;">${report.totalScore}<span style="font-size: 24px; color: #555;">/100</span></div>
          </div>
          
          <h4 style="color: #fff; border-bottom: 1px solid #333; padding-bottom: 12px; margin-bottom: 10px; font-weight: 400; letter-spacing: 1px;">01. DETAIL BREAKDOWN</h4>
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 40px;">
            ${detailsHtml}
          </table>

          <h4 style="color: #fff; border-bottom: 1px solid #333; padding-bottom: 12px; margin-bottom: 20px; font-weight: 400; letter-spacing: 1px;">02. NEXT ACTIONS</h4>
          ${actionsHtml}
          
          <div style="margin-top: 40px; padding: 30px 20px; background-color: #1a1a1a; border-radius: 12px; border: 1px solid #333;">
            <h3 style="color: #fff; margin-top: 0; font-size: 16px; font-weight: normal; margin-bottom: 15px;">💡 さらに詳しい分析や、実装の代行をご希望ですか？</h3>
            <p style="color: #bbb; line-height: 1.6; font-size: 14px; margin-bottom: 10px;">空中八策チームでは、AI時代に勝ち残るためのGoogleビジネスプロフィール最適化から、日々の運用代行までをワンストップでサポートしています。</p>
            <p style="color: #bbb; line-height: 1.6; font-size: 14px; margin-bottom: 25px;">「何から手をつければいいかわからない」「プロに任せたい」という方は、ぜひ一度無料オンライン相談をご利用ください。</p>
            <div style="text-align: center;">
              <a href="https://kuuchuu8sk.co.jp/" style="display: inline-block; background-color: #fff; color: #000; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-weight: bold; font-size: 14px; box-shadow: 0 4px 12px rgba(255,255,255,0.1);">👉 無料オンライン相談を予約する</a>
            </div>
          </div>

          <div style="margin-top: 50px; padding-top: 25px; border-top: 1px solid #222; text-align: center; color: #444; font-size: 11px; letter-spacing: 0.5px;">
            This report was auto-generated by Kuuchuu8sk AI.<br>
            Please note that API data might not reflect real-time updates instantly.<br><br>
            © 2026 Kuuchuu8sk All Rights Reserved.
          </div>
        </div>
      </div>
      `;
      ...
      if (clientMail.error) {
        console.error("[EMAIL] Client mail failed:", clientMail.error);
      }
      ...
      if (process.env.EMAIL_NOTIFICATION_RECEIVER) {
        ...
        if (adminMail.error) {
          console.error("[EMAIL] Admin mail failed:", adminMail.error);
        }
      }
    }
